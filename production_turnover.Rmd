...
ind_chg <- df %>% 
  select(obs_ID, root_ID, Tube, Location, Date, Session, Month, root_status, 
         Length_mm, AvgDiam_cm, Volume_mm3, treatment) %>% 
...
ggplot(gc %>% filter(metric == "length"), aes(Location*-1, value, color = name)) +
  geom_point(alpha = .2, size = .5) +
  geom_smooth(method = "gam", size = .5, show.legend = T, se = F) +
  scale_color_manual(values = c("deepskyblue", "red")) +
  coord_flip() +
  facet_grid(Month ~ treatment, scales = "free_x") +
  xlab("Relative depth") +
  ylab("Gross change") +
  theme_bw() +
  theme(
...
Compare calculated gross change (gain - loss) from T_1_ to T_2_ to the observed total at T_2_ of root length for each window.
### Sanity Check
```{r sanity check gain loss, echo=T, fig.height=4, fig.width=4}
...
ggplot(df2 %>% filter(Location <= 20), aes(x = Location*-1, color = treatment)) +
  geom_point(aes(y = gg_sum), shape = 1, alpha = 1, size = .5) +
  geom_point(aes(y = gl_sum), shape = 2, alpha = 1, size = .5) +
  geom_smooth(aes(y = gg_sum), linetype = 1, method = "gam", size = .5, show.legend = T, se = F) +
  geom_smooth(aes(y = gl_sum), linetype = 2, method = "gam", size = .5, show.legend = T, se = F) +
  # scale_color_manual(values = c("deepskyblue", "red")) +
  coord_flip() +
...
d <- df2 %>% 
  filter(Location <= 20)
library(lme4)
m <- lmer(gl_sum ~ clipped + clipped:manure + (1|Location) + (1|Tube), data = df2)
summary(m)
sjPlot::plot_model(m, type = "re")

m2 <- lmer(gg_sum ~ clipped + clipped:manure + (1|Location) + (1|Tube), data = df2)
summary(m2)
