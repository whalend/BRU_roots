...
volume_tot_lai_d3 <- lmer(tot_volume_mm3 ~ avg_temp + avg_vwc + avg_lai + composition  + (1 | depth) + (1 | Tube) + (1 | month_num), data = depth_lai_model_data)
summary(volume_tot_lai_d3)
sjPlot::plot_model(volume_tot_lai_d3, show.values = T)
# sjPlot::plot_model(volume_tot_lai_d3, type = "re")

ggplot(depth_lai_model_data, aes(avg_lai, tot_volume_mm3)) +
  geom_point(aes(color = clipped, shape = composition), position = "jitter")
```

Next step: Total root volume/length to nearest LAI acquisition date - and potential lag between LAI measurement to root production, or vice versa.
