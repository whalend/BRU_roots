...
roots_data <- left_join(df %>% select(-starts_with("Tot"), -TipDiam:-Altitude), 
                        mr_plot_data)
roots_data <- left_join(roots_data, lai_root, by = c("plot_id", "block", "month_num" = "root_month"))

write_csv(roots_data, "data/roots_treatment_data.csv")
```


## Differences at first sample date
[Issue #4](https://github.com/whalend/BRU_roots/issues/4)
If it's already different in April then the treatments may just be maintaining the differences.
Also could be interpreted as carry-over effect from (treatments) previous year

tubes 6, 15, 23 replaced in April - check these tubes specifically
*don't appear to influence the overall pattern*

```{r compare measurements to April}
depth_april <- depth_data %>% 
  filter(month_num == 4) %>% 
  mutate(Tube = as.character(Tube))
april_means <- depth_april %>% 
  group_by(treatment) %>% 
  summarise(avg_length_mm = mean(tot_length_mm),
            avg_diam_mm = mean(avg_diam_mm),
            avg_volume_mm3 = mean(tot_volume_mm3))
april_means2 <- depth_april %>% 
  filter(Tube!=11) %>% 
  group_by(treatment) %>% 
  summarise(avg_length_mm = mean(tot_length_mm),
            avg_diam_mm = mean(avg_diam_mm),
            avg_volume_mm3 = mean(tot_volume_mm3))
month_means <- depth_data %>% 
  group_by(treatment, month_num) %>% 
  summarise(avg_length_mm = mean(tot_length_mm),
            avg_diam_mm = mean(avg_diam_mm),
            avg_volume_mm3 = mean(tot_volume_mm3))

len <- ggplot(depth_data, aes(Location*-1, tot_length_mm, color = treatment)) +
  # geom_point(aes(Location, tot_length_mm, color = treatment), alpha = .5) +
  geom_smooth(se = FALSE) +
  geom_smooth(data = depth_data %>% filter(!Tube%in%c("11")), se = FALSE, linetype = "dashed") +
  geom_hline(data = april_means, aes(yintercept = avg_length_mm, color = treatment)) +
  geom_hline(data = april_means2, aes(yintercept = avg_length_mm, color = treatment), linetype = "dotted") +
  # geom_hline(data = month_means, aes(yintercept = avg_length_mm, color = treatment), linetype = "dashed") +
  coord_flip() +
  facet_grid(composition~month_num)

diam <- ggplot(depth_data) +
  geom_smooth(aes(Location*-1, avg_diam_mm, color = treatment), se = FALSE) +
  geom_hline(data = april_means, aes(yintercept = avg_diam_mm, color = treatment)) +
  geom_hline(data = month_means, aes(yintercept = avg_diam_mm, color = treatment), linetype = "dashed") +
  coord_flip() +
  facet_grid(month_num~.)

vol <- ggplot(depth_data) +
  geom_smooth(aes(Location*-1, tot_volume_mm3, color = treatment), se = FALSE) +
  geom_hline(data = april_means, aes(yintercept = avg_volume_mm3, color = treatment)) +
  geom_hline(data = month_means, aes(yintercept = avg_volume_mm3, color = treatment), linetype = "dashed") +
  coord_flip() +
  facet_grid(month_num~.)

library(patchwork)

len + xlab("Relative depth") + vol + xlab(" ") + diam + xlab(" ") +
  plot_layout(guides = "collect")
```

```{r model comparing measurements in april}
# depth_data %>% distinct(Tube, block, treatment)# block+Tube = treatment
tot_length_apr_mod <- lmer(tot_length_mm ~ clipped + clipped:manure + (1|Location) + (1|block/Tube), data = depth_april)
summary(tot_length_apr_mod)
tube_re <- sjPlot::plot_model(tot_length_apr_mod, type = "re")[[2]]
tube_re$data <- tube_re$data %>% 
  mutate(Tube = str_sub(term, start = 1, end = -3),
         block = str_sub(term, start = -1, end = -1)) %>% 
  left_join(., depth_april %>% distinct(Tube, treatment))
ggplot(tube_re$data, aes(x = term, shape = treatment, color = block)) +
  geom_point(aes(y = estimate), size = 3) +
  geom_errorbar(aes(ymin = conf.low, ymax = conf.high), size = 1, width = 0) +
  coord_flip()
  

avg_diam_apr_mod <- lmer(avg_diam_mm ~ clipped + clipped:manure + (1|Location) + (1|block/Tube), data = depth_april)
summary(avg_diam_apr_mod)

tot_vol_apr_mod <- lmer(tot_volume_mm3 ~ clipped + clipped:manure + (1|Location) + (1|block/Tube), data = depth_april)
summary(tot_vol_apr_mod)
...
depth_model_data <- depth_data %>% 
  ...
  select(Tube, depth = Location, tot_length_mm:avg_diam_mm, composition,
         clipped, manure, month_num, avg_vwc:avg_lai, LAI, total_wt_g, block, plot_id, treatment) %>% 
...
write_csv(depth_model_data, "data/depth_model_data.csv")
...
len_tot_d2 <- lmer(tot_length_mm ~ min_temp + avg_vwc + clipped + clipped:manure + composition + (1 | depth) + (1 | block/Tube) + (1 | month_num), data = depth_model_data)
len_tot_d3 <- lmer(tot_length_mm ~ avg_temp + avg_vwc + clipped + clipped:manure + composition + (1 | depth) + (1 | block/Tube) + (1 | month_num), data = depth_model_data)
...
re[[1]] + ggtitle("Random effects: depth")
re[[2]] + ggtitle("Random effects: tube")
re[[3]] + ggtitle("Random effects: block")
re[[4]] + ggtitle("Random effects: month")
# Order of plots: depth, tube, month
```
...
Is LAI confounded with the "clipped" treatment? yes. Exclude treatment from models with LAI

```{r depth summed root length LAI}
len_tot_lai_d1 <- lmer(tot_length_mm ~ max_temp + avg_vwc + avg_lai + composition + (1 | depth) + (1 | block/Tube) + (1 | month_num), data = depth_model_data)
summary(len_tot_lai_d1)

sjPlot::plot_model(len_tot_lai_d1, show.values = TRUE)
...
len_tot_lai_d1a <- lmer(tot_length_mm ~ max_temp + avg_vwc + LAI + composition + (1 | depth) + (1 | block/Tube) + (1 | month_num), data = depth_model_data)
summary(len_tot_lai_d1a)
sjPlot::plot_model(len_tot_lai_d1a, show.values = T) +
  ggtitle("Tot length, nearest LAI value")
```

Pretty strong negative relationship with average LAI and maximum temperature but not with nearest LAI value.


#### Summed root length model with mass from clippings
```{r depth summed root length mass}
wt_data <- depth_model_data %>% 
  filter(!is.na(total_wt_g)) %>% 
  group_by(Tube) %>% 
  summarise(wt_g = sum(total_wt_g, na.rm = T))
wt_data <- left_join(depth_model_data, wt_data)
summary(wt_data)

d <- depth_model_data %>% 
  group_by(Tube) %>% 
  summarise(tot_length = sum(tot_length_mm)) %>% 
  ungroup
d <- left_join(d, wt_data) %>% 
  filter(!is.na(wt_g))
m1 <- lmer(tot_length ~ wt_g + composition + (1 | depth) + (1 | month_num) + (1 | block/Tube), data = wt_data)
## Repeat weight values from April across all sampling
len_tot_mass_d1 <- lmer(tot_length_mm ~ max_temp + avg_vwc + wt_g + composition + (1 | depth) + (1 | month_num) + (1 | block/Tube), data = wt_data)
DHARMa::simulateResiduals(len_tot_mass_d1) %>% plot()
summary(len_tot_mass_d1)
sjPlot::tab_model(len_tot_mass_d1)
sjPlot::plot_model(len_tot_mass_d1, type = "est", colors = "bw", show.values = T)
sjPlot::plot_model(len_tot_mass_d1, type = "pred", show.data = T)
# sjPlot::plot_model(len_tot_mass_d1, type = "int", show.data = T)
sjPlot::plot_model(len_tot_mass_d1, type = "re", show.values = T)

## Only April data
len_tot_mass_d2 <- lmer(tot_length_mm ~ max_temp + avg_vwc + total_wt_g + composition + (1 | depth)  + (1 | block/Tube), data = depth_model_data)
summary(len_tot_mass_d2)

## month as factor
len_tot_mass_d3 <- lmer(tot_length_mm ~ max_temp + avg_vwc + wt_g + composition + as.factor(month_num) + (1 | depth) + (1 | block/Tube), data = wt_data)
summary(len_tot_mass_d3)

AIC(len_tot_mass_d1, len_tot_mass_d2, len_tot_mass_d3)

ggplot(wt_data, aes(wt_g, tot_length_mm)) +
  geom_point() +
  geom_smooth() +
  facet_grid(month_num~.) +
  NULL

ggplot(depth_model_data, 
       aes(x = as.factor(month_num), y = tot_length_mm)) + 
  ggdist::stat_halfeye(
    adjust = .5, 
    width = .6, 
    .width = 0, 
...
ggplot(depth_model_data %>% filter(!is.na(total_wt_g)), 
       aes(x = composition, y = total_wt_g)) + 
  ggdist::stat_halfeye(
    adjust = .5, 
    width = .6, 
    .width = 0, 
...
diam_avg_d1 <- lmer(avg_diam_mm ~ max_temp + avg_vwc + clipped + clipped:manure + composition + (1 | depth) + (1 | block/Tube) + (1 | month_num), data = depth_model_data)
...
diam_avg_d3 <- lmer(avg_diam_mm ~ avg_temp + avg_vwc + clipped + clipped:manure + composition + (1 | depth) + (1 | block/Tube) + (1 | month_num), data = depth_model_data)
AIC(diam_avg_d1, diam_avg_d2, diam_avg_d3) %>% 
  arrange(AIC)# kinda all the same

plot(diam_avg_d2)
DHARMa::simulateResiduals(diam_avg_d2) %>% plot()

summary(diam_avg_d2)
sjPlot::plot_model(diam_avg_d2, show.values = T) +
  scale_y_continuous(c(-.1,.1))
re <- sjPlot::plot_model(diam_avg_d2, type = "re")
re[[1]] + ggtitle("Random effects, depth") + scale_y_continuous(c(-.1,.1))
re[[2]] + ggtitle("Random effects, tube:block") + scale_y_continuous(c(-.1,.1))
re[[3]] + ggtitle("Random effects, block") + scale_y_continuous(c(-.1,.1))
re[[4]] + ggtitle("Random effects, month")+ scale_y_continuous(c(-.1,.1))


diam_avg_d2a <- lmer(avg_diam_mm ~ std2_min_temp + std2_avg_vwc + clipped + clipped:manure + composition + (1 | depth) + (1 | block/Tube) + (1 | month_num), data = depth_model_data)
sjPlot::plot_model(diam_avg_d2a, show.values = T) +
  scale_y_continuous(limits = c(-.1, .1)) +
  ggtitle("Avg diameter model with standardized variables")
```

Average root diameter is greater in the mixed versus the bahia-only composition, indicating that peanut roots have larger diameters. The scale of these coefficient estimates is super tiny - 100ths of millimeters...maybe...could be units issue. But as long as any units issue is simply scalar the relationships should be the same.
...
avg_diam_lai_d2 <- lmer(avg_diam_mm ~ min_temp + avg_vwc + avg_lai + composition + (1 | depth) + (1 | block/Tube) + (1 | month_num), data = depth_lai_model_data)
summary(avg_diam_lai_d2)
sjPlot::plot_model(avg_diam_lai_d2, show.values = T) +
  scale_y_continuous(limits = c(-.1,.1))

re <- sjPlot::plot_model(avg_diam_lai_d2, type = "re")
(re[[1]] + ggtitle("Random effects: depth")) + scale_y_continuous(limits = c(-.1,.1))
(re[[2]] + ggtitle("Random effects: tube")) + scale_y_continuous(limits = c(-.1,.1))
(re[[3]] + ggtitle("Random effects: block")) + scale_y_continuous(limits = c(-.1,.1))
(re[[4]] + ggtitle("Random effects: month")) + scale_y_continuous(limits = c(-.1,.1))


avg_diam_lai_d2a <- lmer(avg_diam_mm ~ min_temp + avg_vwc + LAI + composition + (1 | depth) + (1 | Tube) + (1 | month_num), data = depth_lai_model_data)
summary(avg_diam_lai_d2a)
sjPlot::plot_model(avg_diam_lai_d2a, show.values = T) +
  scale_y_continuous(limits = c(-.1,.1)) +
  ggtitle("Avg diam, nearest LAI value")
```

A negative but very small value relationship with avg LAI; no relationship with nearest LAI measurement. 


```{r depth avg root diam mass}
avg_diam_mass_d2 <- lmer(avg_diam_mm ~ min_temp + avg_vwc + wt_g + composition + (1 | depth) + (1 | block/Tube) + (1 | month_num), data = wt_data)
summary(avg_diam_mass_d2)
sjPlot::plot_model(avg_diam_mass_d2, show.values = T) +
  scale_y_continuous(limits = c(-.1,.1))

avg_diam_mass_d2a <- lmer(avg_diam_mm ~ min_temp + avg_vwc + total_wt_g + composition + (1 | depth) + (1 | block/Tube), data = depth_model_data)
summary(avg_diam_mass_d2a)
sjPlot::plot_model(avg_diam_mass_d2a, show.values = T) +
  scale_y_continuous(limits = c(-.1,.1))
```
...
#### Root volume
```{r depth avg root volume models}
## Average root volume at each depth ####
volume_avg_d1 <- lmer(avg_volume_mm3 ~ max_temp + avg_vwc + clipped + clipped:manure + composition + (1 | depth) + (1 | block/Tube) + (1 | month_num), data = depth_model_data)
volume_avg_d2 <- lmer(avg_volume_mm3 ~ min_temp + avg_vwc + clipped + clipped:manure + composition + (1 | depth) + (1 | block/Tube) + (1 | month_num), data = depth_model_data)
volume_avg_d3 <- lmer(avg_volume_mm3 ~ avg_temp + avg_vwc + clipped + clipped:manure + composition + (1 | depth) + (1 | block/Tube) + (1 | month_num), data = depth_model_data)
...
sjPlot::tab_model(volume_avg_d3)
sjPlot::plot_model(volume_avg_d3, show.values = T)
re <- sjPlot::plot_model(volume_avg_d3, type = "re", colors = "bw")
( (re[[1]] + ggtitle("Depth")) |
  (re[[2]] + ggtitle("Tube")) |
  # (re[[3]] + ggtitle("Block")) |
  (re[[4]] + ggtitle("Month")) ) +
...
# volume_avg_d3a <- lmer(avg_volume_mm3 ~ std2_avg_temp + std2_avg_vwc + clipped + clipped:manure + composition + (1 | depth) + (1 | block/Tube) + (1 | month_num), data = depth_model_data)
# summary(volume_avg_d3a)
...
## add LAI into model - subset of data with no NA values for LAI
avg_volume_lai_d3 <- lmer(avg_volume_mm3 ~ avg_temp + avg_vwc + avg_lai + composition +  (1 | depth) + (1 | block/Tube) + (1 | month_num), data = depth_lai_model_data)
...
## Need to think on handling & interpretation of LAI in clipped plots: confounded so remove treatment.

avg_volume_lai_d3a <- lmer(avg_volume_mm3 ~ avg_temp + avg_vwc + LAI + composition +  (1 | depth) + (1 | block/Tube) + (1 | month_num), data = depth_lai_model_data)
summary(avg_volume_lai_d3a)
sjPlot::plot_model(avg_volume_lai_d3a, show.values = T) +
  ggtitle("Avg volume, nearest LAI value")
```

No effect of average or nearest LAI.


```{r depth total root volume, eval=TRUE}
volume_tot_d1 <- lmer(tot_volume_mm3 ~ max_temp + avg_vwc + clipped + clipped:manure + composition + (1 | depth) + (1 | block/Tube) + (1 | month_num), data = depth_model_data)
volume_tot_d2 <- lmer(tot_volume_mm3 ~ min_temp + avg_vwc + clipped + clipped:manure + composition + (1 | depth) + (1 | block/Tube) + (1 | month_num), data = depth_model_data)
volume_tot_d3 <- lmer(tot_volume_mm3 ~ avg_temp + avg_vwc + clipped + clipped:manure + composition + (1 | depth) + (1 | block/Tube) + (1 | month_num), data = depth_model_data)
AIC(volume_tot_d1, volume_tot_d2, volume_tot_d3) %>% 
  arrange(AIC)
plot(volume_tot_d3)
DHARMa::simulateResiduals(volume_tot_d3) %>% plot()

volume_tot_d3 <- lmer(log(tot_volume_mm3) ~ avg_temp + avg_vwc + clipped + clipped:manure + composition + (1 | depth) + (1 | block/Tube) + (1 | month_num), data = depth_model_data)

summary(volume_tot_d3)
sjPlot::plot_model(volume_tot_d3, type = "est", show.values = T)
sjPlot::plot_model(volume_tot_d3, type = "pred", show.data = T)
...
```{r summed volume lai model}
volume_tot_lai_d3 <- lmer(tot_volume_mm3 ~ avg_temp + avg_vwc + avg_lai + composition  + (1 | depth) + (1 | block/Tube) + (1 | month_num), data = depth_lai_model_data)
summary(volume_tot_lai_d3)
sjPlot::plot_model(volume_tot_lai_d3, show.values = T)
# sjPlot::plot_model(volume_tot_lai_d3, type = "re")

ggplot(depth_lai_model_data, aes(clipped, avg_lai)) +
  geom_boxplot() +
  geom_point(aes(color = clipped, shape = composition), position = "jitter") 

volume_tot_lai_d3a <- lmer(tot_volume_mm3 ~ avg_temp + avg_vwc + LAI + composition  + (1 | depth) + (1 | block/Tube) + (1 | month_num), data = depth_lai_model_data)
summary(volume_tot_lai_d3a)
sjPlot::plot_model(volume_tot_lai_d3a, show.values = T) +
  ggtitle("Total vol, nearest LAI value")
```

Negative relationship between average LAI and total root volume; no relationship with nearest LAI value.
...
tube_model_data <- tube_data %>% 
  ...
  select(Tube, tot_length_mm:avg_diam_mm, composition, clipped, manure,
         month_num, avg_vwc:avg_sem, LAI, block, plot_id, treatment) %>% 
...
#### Average root volume
```{r tube avg volume}
vol_avg_m1 <- lmer(avg_volume_mm3 ~ max_temp + avg_vwc + clipped + clipped:manure + composition + (1|block/Tube) + (1|month_num), data = tube_model_data)
...
#### Summed root volume
```{r tube summed volume, eval=TRUE}
...
AIC(vol_tot_m1, vol_tot_m2, vol_tot_m3) %>% 
  arrange(AIC)# avg temp ever so slightly < min temp
summary(vol_tot_m3)
plot(vol_tot_m3)
...
Present both analyses (average and summed forms)
1. root size characteristics
2. how much root is there
3. individual roots?

Avg diameter
Avg vs. total volume comparison

Check individual date acquisitions of LAI
Use plot biomass clipping data instead of LAI
 - use pmass (lower of two variables)
 
glmer(family = Gamma) for biomass

Why no treatment effect on gross gain & gross loss compared to total/average?

Check for differences at first date - if it's already different in April then the treatment may just be maintaining
