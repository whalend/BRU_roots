...
len <- ggplot(depth_data) +
  # geom_point(aes(Location, tot_length_mm, color = treatment), alpha = .5) +
  geom_smooth(aes(Location*-1, tot_length_mm, color = treatment), se = FALSE) +
...
depth_model_data <- depth_data %>% 
  ...
  select(Tube, depth = Location, tot_length_mm:avg_diam_mm, composition,
         clipped, manure, month_num, avg_vwc:avg_sem, LAI, block, plot_id, treatment) %>% 
...
```{r depth avg root length LAI, eval=F}
...
## depth avg root length nearest lai
avg_len_lai_d1a <- lmer(avg_length_mm ~ max_temp + avg_vwc + LAI + composition + (1 | depth) + (1 | Tube) + (1 | month_num), data = depth_lai_model_data)
summary(avg_len_lai_d1a)
sjPlot::plot_models(avg_len_lai_d1, avg_len_lai_d1a, show.values = TRUE)
```
